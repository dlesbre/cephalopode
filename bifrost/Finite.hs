...
-- Analyze a BitefulProgram and determine what values are possible for each
...
analyzeFinite :: BitefulProgram -> Environment -> M.Map Var (S.Set Exp)
analyzeFinite prog env =
  let vars = M.keys $ env_vars env
      initial = M.fromList $ map (\v -> (v, S.empty)) vars
      analyze = mapM_ analyzeBiteful $ M.elems $ bitp_bites prog
  in execState analyze initial

type Work = State (M.Map Var (S.Set Exp)) ()

analyzeBiteful :: Biteful -> Work
analyzeBiteful bite = mapM_ foo $ M.toList $ bite_updates bite
  where
    foo (var, exp) =
  ...
  where
    foo exp =
      case getInner exp of
        ...
        EBiteLabel _ -> return $ S.singleton exp
