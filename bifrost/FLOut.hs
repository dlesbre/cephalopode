...
flModule mod = do
  ...
  let decls = ["// common"] ++ commonDecls ++
              ...
              ["// internal"] ++ intDecls
  let cellBegin = ["CELL \"draw_hier " ++ name ++"\" ["]
  cellInsts <- mapM flInst $ mod_others mod
  cellFlops <- mapM flFlop $ mod_flops mod
  cellEquations <- mapM flEquation $ M.toList $ mod_equations mod
...
flInst :: ModuleInstantiation -> F String
flInst (ModuleInstantiation name wires) = do
  wires' <- mapM flWire $ map (\(_,w,_)->w) common ++ wires
  return $ unwords $ name:wires'

-- Create flip-flop instantiations, e.g. "re_ff clk ack was_ack".
flFlop :: Floppish -> F String
flFlop flop = do
  ...
  (variety, args) <-
        case (maybeEn, maybeRst, flop_transparent flop) of
          ...
          (Nothing, Just rst, False) -> return ("_reset", [clk, rst, din, dout])
          ...
          (Just en, Just rst, False) -> return ("_en_reset", [clk, en, rst, din, dout])
          (Just en, Just rst, True) -> return ("_en_reset_mux", [clk, en, rst, din, dout])
  return $ unwords $ ("re_ff" ++ variety):args
  where
...
-- type.
flEquation :: (Wire, Exp) -> F String
flEquation (wire, exp) = do
  lhs <- flWire wire
  rhs <- flExp exp
  let typ = typeOf exp
...
flEnum :: (TypeName, [EnumValue]) -> F String
flEnum (name, values) = do
