...
initArrState env = ArrState env M.empty M.empty NoClue False False
...
crawlDecl decl =
  case decl of
    ...
    Ast.ActionDecl name typ provider protocol ->
      let name' = convertActionName name
          typ' = convertActionTypeName typ
          provider' = convertProvider provider
          protocol' = convertProtocol protocol
      in declareAction name' typ' provider' protocol'
...
crawlStm sub stm =
  case stm of
    ...
    Ast.Abnormal (Ast.IfElse exp blk1 blk2) -> do
      exp' <- crawlExp sub exp
      blk1' <- crawlStmBlock sub blk1
      blk2' <- crawlStmBlock sub blk2
      return $ Branch exp' blk1' blk2'
    Ast.Abnormal (Ast.IfOnly exp blk) -> do
      exp' <- crawlExp sub exp
...
crawlExpNoExpand :: Ast.Exp -> Arr Exp
crawlExpNoExpand astexp =
  case astexp of
...
crawlCall call =
  case call of
    Ast.ActionCall name args -> do
      ...
      args' <- mapM crawlExpNoExpand $ unargs args
        ...
        Just (params, replacement) ->
          ...
          in if arity <= length spine
            then do
              ...
              replacement' <- crawlExpNoExpand replacement
...
declareAction
  ...
  -> Protocol
  -> Arr ()
declareAction name typ prov proto = do
  env <- getEnv
  let actions = env_actions env
  let actionOrder = env_actionOrder env
  when (M.member name actions) $ yell $ errRedeclare "action" name
  when (isNothing $ M.lookup typ $ env_actionTypes env) $ yell $
    errNo "action type" typ
  let actions' = M.insert name (ActionInfo typ proto prov) actions
  let actionOrder' = actionOrder ++ [name]
  putEnv $ env { env_actions = actions', env_actionOrder = actionOrder' }

declareActionType :: ActionTypeName -> ActionType -> Arr ()
declareActionType name typ = do
  env <- getEnv
  let ats = env_actionTypes env
  when (M.member name ats) $ yell $ errRedeclare "action type" name
  let ats' = M.insert name typ ats
  let env' = env { env_actionTypes = ats' }
  putEnv env'
...
convertCommProtocol :: Ast.CommProtocol -> CommProtocol
convertCommProtocol proto =
  case proto of
    Ast.CommProtocolTwoPhase -> TwoPhase
    Ast.CommProtocolFourPhase -> FourPhase
    Ast.CommProtocolPulseEcho -> PulseEcho
    Ast.CommProtocolValidReady -> ValidReady
    Ast.CommProtocolCombinational -> Combinational 0
...
convertProtocol :: Ast.Protocol -> Protocol
convertProtocol proto =
  case proto of
    Ast.ProtocolAuto -> ProtocolAuto
    Ast.ProtocolGiven comm pwr ->
      let pwr' = convertPowerProtocol pwr
          comm' = convertCommProtocol comm
          needsClkReset = case (pwr', comm') of
            (AlwaysOn, Combinational _) -> False
            _ -> True
      in Protocol needsClkReset needsClkReset pwr' comm'

convertProvider :: Ast.Provider -> ActionProvider
convertProvider prov =
  case prov of
    Ast.ProviderExternal -> External
    Ast.ProviderModule s -> Instantiate s
